classdef CocoUtils_building
    ...
    % Help on each functions can be accessed by: "help CocoUtils_building>function".
    %
    % See also CocoApi MaskApi CocoEval CocoUtils_building>convertPascalGt
    % CocoUtils_building>convertImageNetGt CocoUtils_building>convertPascalDt
    % CocoUtils_building>convertImageNetDt CocoUtils_building>validateOnPascal
    % CocoUtils_building>validateOnImageNet CocoUtils_building>generateFakeDt
    % CocoUtils_building>validateMaskApi CocoUtils_building>gasonSplit CocoUtils_building>gasonMerge
    %
    ...
    methods( Static )
        function convertPascalGt( dataDir, year, split, annFile )
            ...
            dev=[dataDir '/']; addpath(genpath([dev 'VOCcode']));
            ...
            data=CocoUtils_building.initData(C,n);
            for i=1:n, nm=[is{i} '.jpg'];
                ...
                O.class;
                t=catsMap.values({O.class});
                catIds=[t{:}];
                iscrowd=ignore*0;
                data=CocoUtils_building.addData(data,nm,id,hw,catIds,ignore,iscrowd,bbs);
            end
            annFile
            f=fopen(annFile,'w')
            fwrite(f,gason(data))
        ...
        function convertImageNetGt( dataDir, year, split, annFile )
            ...
            %
            % USAGE
            %  CocoUtils_building.convertImageNetGt( dataDir, year, split, annFile )
            %
            % INPUTS
            ...
            data=CocoUtils_building.initData(catNms,n);
            for i=1:n
                ...
                data=CocoUtils_building.addData(data,nm,i,hw,catIds,ignore,iscrowd,bbs);
            end
            f=fopen(annFile,'w');
            fwrite(f,gason(data));
            fclose(f);
        ...
        function convertPascalDt( srcFiles, tarFile )
            ...
            %
            % USAGE
            %  CocoUtils_building.convertPascalDt( srcFiles, tarFile )
            %
            % INPUTS
        ...
        function convertImageNetDt( srcFile, tarFile )
            ...
            %
            % USAGE
            %  CocoUtils_building.convertImageNetDt( srcFile, tarFile )
            %
            % INPUTS
        ...
        function validateOnPascal( dataDir )
            ...
            %
            % USAGE
            %  CocoUtils_building.validateOnPascal( dataDir )
            %
            % INPUTS
            %  dataDir    - dir containing VOCdevkit/
            ...
            CocoUtils_building.convertPascalGt(dataDir,year,split,annFile);
            CocoUtils_building.convertPascalDt(sfs,resFile);
        ...
        function validateOnImageNet( dataDir )
            ...
            %
            % USAGE
            %  CocoUtils_building.validateOnImageNet( dataDir )
            %
            % INPUTS
            %  dataDir    - dir containing ILSVRC*/ folders
            ...
            CocoUtils_building.convertImageNetDt(fs{1},resFile);
            CocoUtils_building.convertImageNetGt(dataDir,year,'val',annFile)
            D=CocoApi(annFile); R=D.loadRes(resFile); E=CocoEval(D,R);
        ...
        function generateFakeDt( coco, dtFile, varargin )
            ...
            %
            % USAGE
            %  CocoUtils_building.generateFakeDt( coco, dtFile, varargin )
            %
            % INPUTS
        ...
        function validateMaskApi( coco )
            ...
            %
            % USAGE
            %  CocoUtils_building.validateMaskApi( coco )
            %
            % INPUTS
            %  coco       - instance of CocoApi containing ground truth
        ...
        function gasonSplit( name, k )
            ...
            %
            % USAGE
            %  CocoUtils_building.gasonSplit( name, k )
            %
            % INPUTS
        ...
        function gasonMerge( name )
            ...
            %
            % USAGE
            %  CocoUtils_building.gasonMerge( name )
            %
            % INPUTS
    ...
    methods( Static, Access=private )
        ...
        function data = addData( data,nm,id,hw,catIds,ignore,iscrowd,bbs )
            ...
            %%%jky%%%
            pointpath='../VOCdevkit/VOC0712/points/';
            pfilename=[pointpath,nm(1:end-4),'.txt'];
            points=fopen(pfilename,'r');
            % pwd
            for j=1:length(catIds), data.nAnns=data.nAnns+1; k=data.nAnns;
                b=bbs(j,:); b(3:4)=b(3:4)-b(1:2);
                % x1=b(1); x2=b(1)+b(3); y1=b(2); y2=b(2)+b(4);
                point=str2num(fgetl(points));
                S={{point}}; a=b(3)*b(4);
